...
import (
	...
	"github.com/some-things/ent-eval/ent/group"
...
func main() {
	...
	// Query cars of users in group github
	if err := QueryGithub(ctx, client); err != nil {
		log.Fatalf("failed querying github: %v", err)
	}

	// Query cars of users in Ariel's groups that aren't model == Mazda
	if err := QueryArielCars(ctx, client); err != nil {
		log.Fatalf("failed querying cars: %v", err)
	}

	// Query groups that have users
...
func QueryGroupWithUsers(ctx context.Context, client *ent.Client) error {
	groups, err := client.Group.
		...
		All(ctx)
	if err != nil {
		return fmt.Errorf("failed querying groups: %w", err)
	}
